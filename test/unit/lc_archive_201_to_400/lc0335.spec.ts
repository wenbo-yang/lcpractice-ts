xdescribe('leetcode 335: description', () => {
    ...
    function testTwoVectors(testVec: number[][], crossVec: number[][]): boolean {
        ...
        const firstBound = testVec[0][0] === testVec[1][0] ? 
            (intersection[1] <= Math.max(testVec[0][1], testVec[1][1]) && intersection[1] >= Math.min(testVec[0][1], testVec[1][1])) :
            (intersection[0] <= Math.max(testVec[0][0], testVec[1][0]) && intersection[0] >= Math.min(testVec[0][0], testVec[1][0]));

        const secondBound = crossVec[0][0] === crossVec[1][0] ? 
            (intersection[1] <= Math.max(crossVec[0][1], crossVec[1][1]) && intersection[1] >= Math.min(crossVec[0][1], crossVec[1][1])) :
            (intersection[0] <= Math.max(crossVec[0][0], crossVec[1][0]) && intersection[0] >= Math.min(crossVec[0][0], crossVec[1][0]));
