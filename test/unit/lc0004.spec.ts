xdescribe('leetcode 4: find median of two sorted arrays', () => {
    ...
    function findMedianSortedArrays(nums1: number[], nums2: number[]): number {
        ...
        while (l < r) {
            const m1 = l + (r - l) / 2;
            const m2 = k - m1;
            if (smaller[m1] < bigger[m2 - 1]) l = m1 + 1;
            else r = m1;
        }
        ...
        const c1 = Math.max(m1 <= 0 ? Number.MIN_SAFE_INTEGER : smaller[m1 - 1], m2 <= 0 ? Number.MIN_SAFE_INTEGER : bigger[m2 - 1]);

        if ((smaller.length + bigger.length) % 2 === 1) return c1;

        const c2 = Math.min(m1 >= smaller.length ? Number.MAX_SAFE_INTEGER : smaller[m1], m2 >= bigger.length ? Number.MAX_SAFE_INTEGER : bigger[m2]);
