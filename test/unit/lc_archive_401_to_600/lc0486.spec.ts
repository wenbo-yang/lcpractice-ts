xdescribe('leetcode 486: predict winner', () => {
    ...
    function canWinHelper(nums: number[], start: number, end: number, myScore: number, myOpponentsScore: number, sum: number, myTurn: boolean, mem: Map<string, boolean>): boolean {
        ...
        if (myTurn) {
            result = canWinHelper(nums, start + 1, end, myScore + (myTurn ? nums[start] : 0), myOpponentsScore + (myTurn ? 0 : nums[start]), sum - nums[start], !myTurn, mem) || canWinHelper(nums, start, end + 1, myScore + (myTurn ? nums[end] : 0), myOpponentsScore + (myTurn ? 0 : nums[end]), sum - nums[end], !myTurn, mem);
        } else {
            result = canWinHelper(nums, start + 1, end, myScore + (myTurn ? nums[start] : 0), myOpponentsScore + (myTurn ? 0 : nums[start]), sum - nums[start], !myTurn, mem) && canWinHelper(nums, start, end + 1, myScore + (myTurn ? nums[end] : 0), myOpponentsScore + (myTurn ? 0 : nums[end]), sum - nums[end], !myTurn, mem);
        }
